...
def search_engine(text) :
    ...
    driver.implicitly_wait(30) # 을지로3가 맛집 검색의 경우 -> 카페 키워드를 이용한 검색보다 정보를 찾는데 더 많은 시간이 걸리게됨. 따라서 wait 시간이 짧을경우 데이터를 못받아오는 경우가 있음. (따라서 3 -> 30 으로 늘려주었음)
    ...
    shop_id = []
    for i in range(len(shop)) :   # attrs 속성을 이용하면 특정 elements에 접근가능하다. 접근한 최종 데이터에 [1:] 슬라이싱을 통해 앞에 's'를 삭제해줌 
